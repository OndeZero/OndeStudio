...
OndeStudio (the OndeZero studio) is **in build**. Landed: **M0** skeleton, **M1** week grid (team-validated 2026-07-06), **M2** (auth, board, show hub, media browser), **M4** (broadcaster main/test fan-out), reversible negotiation states (ADR-0012), and **M3** the **driver** (write-back: validated weekly show/series/echo slots → tagged AzuraCast playlists with schedule_items; projection ledger + drift engine + reconciliation inbox; RFC 0001). **All four phase-1 exit-bar items now exist** (grid, board, broadcaster mgmt, write-back). AzuraCast writes target `wz-test` only until the dedicated API account + adoption (docs/2 §7.7); everything write-related is live-verified there with tagged cleanup. Next: **M5** (OndePlayer reads OndeStudio's `schedule`/`now`) and the fast-follow list (docs/2 §11) — month/3-day zooms, on-air view, external self-service, episode-queue, live-slot projection, now-playing meta push (server-side handoff, see the playout README). The specification set under `docs/` remains the spec; RFCs in `docs/rfc/`.
